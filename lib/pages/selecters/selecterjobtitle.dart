...
class Selecterjobtitle extends StatefulWidget {
  ...
  @override
  _SelecterjobtitleState createState() => _SelecterjobtitleState();
}

class _SelecterjobtitleState extends State<Selecterjobtitle> {
    ...
    return
      Scaffold(
        appBar: new AppBar(
          ...
          title: Text(
            allTranslations.text("Select Job Title"),
...
//print(client);
  ...
  //  all needs variable
  List<Mfilterjobtitle> _recordsdata = [];
  int _currentPage = 1;
...
//if user pull to refresh page
  ...
  _getrecords({ int page : 1 ,int daste : 9, bool refresh : false}) async
  {
    ...
    var response = await classfilterservices.actiongetjobtitlefilters(showname);
    setState(() {
      _recordsdata.clear();
...
//  getting data from model
  ...
  getclientname() async {
    ...
    String url = 'https://mijnzpc.com/api/v1/jobtitleinfo?token=' + token + '&id=' + id;
    ...
    if (response.statusCode == 200) {
      ...
      {
        // setState(() {
        //    showname = responseBody['jobtitle'][0]['title'];
